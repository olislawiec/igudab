...
public class Table {
	...
	private int players;
	private String Messange; // tego msg wywalac do sendAll(); dopisywac tutaj
								// tylko publiczne wiadomosci
	private int currentPlayer = 1; // metoda getCurrentPlayer;
	private int tura; // metode zwiekszajaca ture i metode zwracajaca wartosc
						// tury
	...
	private void changeCard(int player, int card)// prywatna klasa do zmiany
													// karty
	{
		int cardP = (int) ((4 * player) + (card - 4));
		if (returnCards.size() > (50 - (players * 4)))
			rCardToDeck();
	...
	/*
	 * pobieranie maxBet pobieranie playerBet
	 */
	...
	public void bet(int player, String action) {
		...
		case 'C':
			...
			break;
		case 'R':
			Messange = "Player:" + player + " - Rise"
					+ Integer.parseInt(action.substring(1));
