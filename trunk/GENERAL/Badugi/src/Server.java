 ...
 * 
 */
/**Klasa główna stworzona do utworzenia Servera */
class Server
{
    ...
    public static void main(String args[]) throws Exception
    {
		...
		//int playersQuantity=Integer.parseInt(args[1]); //zmienic na tasowanie kart w decku
		//int tokensQuantity=Integer.parseInt(args[2]);
		try{
			System.out.println("ODPALAM SERVA");
			//a=new Deck();
		}catch(Exception e){}
    }
		...
		public void run()
		{
			try{
				while(true)
				{
					...
					try
					{	if(true){//tutaj mozna ustalic kiedy ma przestac odbierac dane od klientow(np zerwanie polaczenia wiekszosci graczy)
						/**Obsluga Tokenow do rozrozniania OP od NUM */
						InetAddress localdr=InetAddress.getLocalHost();
						byte[] hostdr=localdr.getAddress();
/*ciekawe czemu*/		System.out.println("Oczekuje na klienta tutaj:"+localdr+"::"+hostdr+"\n<Aby przerwac wcisnij ctrl+C.>\n");}
						String msgFromClient=new String(); //TYM BEDZIEMY PRZEKAZYWALI TEN TOKEN TURY I KARTY
						msgFromClient=din.readUTF();
						System.out.println(msgFromClient); //TUTAJ ODBIERAMY KARTY Z DANEJ TURY(od klienta)
						...
						String msg1="";
						String msg2="";
						//NA WZOR TEGO NIZEJ DAM ROZMOWE KLIENTA Z SERVEREM.
						/*if(MsgType.equals("Q")) // TE CZESCI RACZEJ SIE NAM NIE PRZYDADZA Z TYMI OPCJAMI Q P R itd...
						{
							...
							/*		else if(MsgType.equals("P")){
									...
									if(iCount==LoginNames.size())
									{
										...
										dout.writeUTF("O "+" "+(msg2)); // tutaj podmienic najpierw na porownanie zestawow kart z rundy a potem rozeslanie nowych
						...
						//else
							...
							{
								System.out.println("Wysylam wynik.");
								//TUTAJ TEZ DECK ZAMIAST TEGO NIZEJ
								//dout.writeUTF("O "+RozkladLiczbyTest.hain(msg2));
								dout.writeUTF("O "+" "+(msg2));
								System.out.println("Zakonczono dzialanie.</"+Sendto+">");
							}
							else
							{
